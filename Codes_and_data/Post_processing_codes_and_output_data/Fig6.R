...
strat_ag <- read.csv("../Data_for_plotting/stratification_sevan_stats_01.csv")
ice <- read.csv("../Data_for_plotting/ice_ag.csv")
ice_ag <- read.csv("../Data_for_plotting/ice_ag_01.csv")

strat_ag$variable <- factor(strat_ag$variable, levels=c("StratStart","StratEnd" ,"MaxStratDur" ,"Schmidt_Stability"  ))


# define own colors
mycol <- c( "#737373","#00441B", "#3690C0", "#CC4C02" )
...
p1 <- strat_ag %>% filter(model == "Ensemble_Mean"& variable %in% c("StratStart")) %>%
  ...
  scale_color_manual("Period", values = mycol, guide = "none",
                     labels=c("Historical", "RCP2.6", "RCP4.5", "RCP8.5")) +
  scale_fill_manual("Period", values = mycol2, guide = "none")+ 
  labs(y = NULL, x = NULL)+ ylim(-50, 20) + ylab("Stratification Onset Anomaly (d)") +
  annotate("text", x=1965, y=20, label= "(A)",size = 5,hjust = 0)
...
p2 <- strat_ag %>% filter(model == "Ensemble_Mean"& variable %in% c("StratEnd")) %>%
  ...
  scale_fill_manual("Period", values = mycol2, guide = "none")+ 
  labs(y = NULL, x = NULL)+ylim(-15,40) + ylab("Stratification End Anomaly (d)") +
  annotate("text", x=1965, y=40, label= "(B)",size = 5,hjust = 0)




p3 <- ice_melted %>% filter(model  == "Ensemble_Mean" & variable %in% c("MaxIceDur")) %>%
  ggplot() +
  ...
  theme_bw(base_size = 20) +
  theme(panel.background = element_rect(colour = 'black')) +
  labs(y = NULL, x = NULL)+ylim(0, 160)+ylab("Max Ice Duration (d)") +
  annotate("text", x=1965, y=160, label= "(C)",size = 5,hjust = 0)



p4 <- ice_melted %>% filter(model  == "Ensemble_Mean" & variable %in% c("HiceMax")) %>%
  ggplot() +
  geom_point(aes(x = date, y =  value, group = interaction(gc_rc, period), col = period), alpha = 0.25)+
  ...
  theme_bw(base_size = 20) +
  theme(panel.background = element_rect(colour = 'black')) +
  labs(y = NULL, x = NULL)+ylim(0,0.45)+ylab("Max Ice thickness (m)")+
  annotate("text", x=1965, y=0.45, label= "(D)",size = 5,hjust = 0)+ ylim(0,0.45)
...
p_legend <- ice_melted %>% filter(model  == "Ensemble_Mean" & variable %in% c("HiceMax")) %>%
  ggplot() +
  geom_point(aes(x = date, y =  value, group = interaction(gc_rc, period), col = period), alpha = 0.25)+
  geom_point(data = ice_ag %>%  filter(model  == "Ensemble_Mean"& variable %in% c("HiceMax")),
             aes(x = date, y = median, group = period, col = period),size = 1.5, stroke =1.5)+
  ...
  theme(panel.background = element_rect(colour = 'black')) +
  labs(y = NULL, x = NULL)+
  annotate("text", x=2098, y=0.45, label= "(D)",size = 5)+ ylim(0,0.45)
...
ggsave('../Main_Manuscript_figures/Fig8_strat_ice_01tt_colorblind.pdf',
       final_plot, width = 350,height = 250, units = 'mm')
