...
air_df <- read.csv("../Data_for_plotting/airtemp_mdelted.csv")
...
# define own colors
#mycol <- c( "#737373","#00441B", "#3690C0", "#FB6A4A" )
...
p1 <- dat_anomaly_withmean %>% filter(variable == "Air_Temperature") %>%
  ...
  theme_bw(base_size = 17) + theme(plot.margin = margin(0, 0, 0, 0),
                                        legend.position="none") +
  labs(x = NULL, y =NULL) +
  scale_color_manual("Period", values = mycol, guide = "none") +
  annotate("text", x=1985, y=6.5, label= "(A)  Air Temperature",size = 5)
...
p2 <- dat_last10 %>% filter(variable == "Air_Temperature") %>% ggplot() +
  ...
  annotate("text", x=-0.25, y=6.5, label= "(B)",size = 5)
...
p3 <- dat_anomaly_withmean %>% filter(model  == "Ensemble_Mean" & variable == "Surface_Temperature") %>%
  ...
  labs(x = NULL, y=NULL)  +
  scale_color_manual("Period", values = mycol, guide = "none")  +
  annotate("text", x=1985, y=6.5, label= "(C)  Surface Temperature",size = 5)
...
p4 <- dat_last10 %>% filter(variable == "Surface_Temperature") %>% ggplot() +
  ...
  scale_colour_manual(values = mycol, guide = "none") +
  scale_fill_manual("Period", values = mycol2, guide = "none")+
  annotate("text", x=-0.25, y=6.5, label= " (D)",size = 5)
...
p5<- dat_anomaly_withmean %>% filter(model  == "Ensemble_Mean" & variable == "Bottom_Temperature") %>%
  ...
  labs( y = NULL, x="Date") +
  scale_color_manual("Period", values = mycol, guide = "none")  +
  annotate("text", x=1985, y=6.5, label= "(E)  Bottom Temperature",size = 5)
...
p6 <- dat_last10 %>% filter(variable == "Bottom_Temperature")%>% ggplot() +
  ...
  scale_colour_manual(values = mycol, guide = "none") +
  scale_fill_manual("Period", values = mycol2, guide = "none") +
  annotate("text", x=-0.25, y=6.5, label= " (F)",size = 5)


pp <- p1+p2+p3+p4+p5+p6+plot_layout(widths = c(3.5, 1))
p_lab <- 
  ggplot() + 
  annotate(geom = "text", x=1,y=1,label = "Anomaly (K)", angle = 90, size=6) +
...
ggsave('../Main_Manuscript_figures/Fig5_warming_air_ensemblemean_noendinforcolorblind3.pdf', my_plot,
       dpi = 300,width = 300, height = 450, units = 'mm')
