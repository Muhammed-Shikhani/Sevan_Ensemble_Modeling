...
txt_mv <- c(-0.37,-0.25,-0.12,.01,0.13,0.25)
...
p1<- dat_anomaly_withmean %>% filter(model  != "Ensemble_Mean" & variable == "Surface_Temperature" &
  ...
  theme_bw()+ 
  theme(text = element_text(size=20),plot.margin = margin(0, 0, 0.25, 0),legend.position="none")+
...
p2<- dat_last10  %>% filter(variable == "Surface_Temperature" &
  ...
  theme_bw()+geom_text(data=dat_last10_mean  %>% filter(variable == "Surface_Temperature" &
                                                          period %in% c("rcp85"))
                       ,aes(x=mean+2.2, y=txt_mv, label=model, colour=model),
                       size=5, angle = 90)+
  guides( y = "none",fill="none", x = "none")+
  labs(x = NULL, y = NULL)+  theme(plot.margin = margin(0, 0.25, 0.25, 0),legend.position="none",
...
p3<- dat_anomaly_withmean %>% filter(model  != "Ensemble_Mean" & variable == "Surface_Temperature" &
  ...
  geom_line( data = dat_an_ag_mean %>% filter(variable == "Surface_Temperature" &
             ...
             aes(x=date, y=mean, group= interaction(model,period), colour=model), size = 1.2)+
  scale_color_jama()+
  scale_y_continuous(limits = c(-2, 5),breaks=seq(-2,5,1), labels=seq(-2,5,1))+
  theme_bw()+ 
  theme(text = element_text(size=20),plot.margin = margin(0, 0, 0.25, 0),legend.position="none")+
  labs(x = NULL, y =NULL)+
  annotate("text", x=1980, y=5, label= "(B)  RCP 4.5",size = 5)
...
p4<- dat_last10  %>% filter(variable == "Surface_Temperature" &
  ...
  coord_flip()+ 
  scale_color_jama()+scale_fill_jama()+
  scale_x_continuous(limits = c(-2, 5),breaks=seq(-2,5,1), labels=seq(-2,5,1))+
  theme_bw()+geom_text(data=dat_last10_mean  %>% filter(variable == "Surface_Temperature" &
                                                          period %in% c("rcp45"))
                       ,aes(x=mean+2, y=txt_mv, label=model, colour=model), size=5, angle = 90)+
  guides( y = "none",fill="none", x = "none")+
  labs(x = NULL, y = NULL)+  theme(plot.margin = margin(0, 0, 0.25, 0.25),legend.position="none",
                                   axis.text.x = element_blank(),
                                   panel.grid.major.x = element_blank(),
                                   panel.grid.minor.x = element_blank(),
                                   axis.ticks.x = element_blank()) +
...
p5<- dat_anomaly_withmean %>% filter(model  != "Ensemble_Mean" & variable == "Surface_Temperature" &
  ...
  geom_line( data = dat_an_ag_mean %>% filter(variable == "Surface_Temperature" &
             ...
             aes(x=date, y=mean, group= interaction(model,period), colour=model), size = 1.2)+
  scale_color_jama()+
  scale_y_continuous(limits = c(-2, 4),breaks=seq(-2,4,1), labels=seq(-2,4,1))+
  theme_bw()+ 
  theme(text = element_text(size=20),plot.margin = margin(0, 0, 0.25, 0),legend.position="none")+
  labs(x = NULL, y =NULL)+
  annotate("text", x=1980, y=4, label= "(C)  RCP 2.6",size = 5)
...
p6<- dat_last10  %>% filter(variable == "Surface_Temperature" &
  ...
  coord_flip()+ 
  scale_color_jama()+scale_fill_jama()+
  scale_x_continuous(limits = c(-2, 4),breaks=seq(-2,4,1), labels=seq(-2,4,1))+
  theme_bw()+geom_text(data=dat_last10_mean  %>% filter(variable == "Surface_Temperature" &
                                                          period %in% c("rcp26"))
                       ,aes(x=mean+1.9, y=txt_mv, label=model, colour=model), size=5, angle = 90)+
  guides( y = "none",fill="none", x = "none")+
  labs(x = NULL, y = NULL)+  theme(plot.margin = margin(0, 0, 0.25, 0.25),legend.position="none",
                                   axis.text.x = element_blank(),
                                   panel.grid.major.x = element_blank(),
...
p7 <- dat_anomaly_withmean %>% filter(model  != "Ensemble_Mean" & variable == "Bottom_Temperature" &
  ...
  scale_y_continuous(limits = c(-2, 8),breaks=seq(-2,8,2), labels=seq(-2,8,2))+
  theme_bw()+ 
  theme(text = element_text(size=20),plot.margin = margin(0, 0.25, 0.25, 0),legend.position="none")+
...
p8 <- dat_last10  %>% filter(variable == "Bottom_Temperature" &
  ...
  theme_bw()+geom_text(data=dat_last10_mean  %>% filter(variable == "Bottom_Temperature" &
                                                          period %in% c("rcp85"))
                       ,aes(x=mean+3, y=txt_mv, label=model, colour=model), size=5, angle = 90)+
  guides( y = "none",fill="none", x = "none")+
  labs(x = NULL, y = NULL)+ theme(plot.margin = margin(0, 0, 0.25, 0),legend.position="none",
                                  axis.text.x = element_blank(),
                                  panel.grid.major.x = element_blank(),
                                  panel.grid.minor.x = element_blank(),
                                  axis.ticks.x = element_blank())+
...
p9 <- dat_anomaly_withmean %>% filter(model  != "Ensemble_Mean" & variable == "Bottom_Temperature" &
  ...
  theme(text = element_text(size=20),plot.margin = margin(0, 0.25, 0.25, 0),legend.position="none")+
  labs(x = NULL, y =NULL)+
  annotate("text", x=1980, y=5, label= "(E)  RCP 4.5",size = 5)
...
p10 <- dat_last10  %>% filter(variable == "Bottom_Temperature" &
  ...
  labs(x = NULL, y = NULL)+theme(plot.margin = margin(0, 0, 0.25, 0),legend.position="none",
                                 ...
                                 panel.grid.major.x = element_blank(),
                                 panel.grid.minor.x = element_blank(),
                                 axis.ticks.x = element_blank())+  geom_vline(xintercept = 0)
...
p11 <- dat_anomaly_withmean %>% filter(model  != "Ensemble_Mean" & variable == "Bottom_Temperature" &
  ...
  theme(text = element_text(size=20),plot.margin = margin(0, 0.25, 0.25, 0),legend.position="none")+
  labs(x = NULL, y =NULL)+
  annotate("text", x=1980, y=4, label= "(F)  RCP 2.6",size = 5)
...
p12 <- dat_last10  %>% filter(variable == "Bottom_Temperature" &
  ...
  theme_bw()+geom_text(data=dat_last10_mean  %>% filter(variable == "Bottom_Temperature" &
                                                          period %in% c("rcp26"))
                       ,aes(x=mean+1.7, y=txt_mv, label=model, colour=model), size=5, angle = 90)+
  guides( y = "none",fill="none", x = "none")+
  labs(x = NULL, y = NULL)+  theme(plot.margin = margin(0, 0, 0.25, 0),legend.position="none",
                                   axis.text.x = element_blank(),
                                   panel.grid.major.x = element_blank(),
...
ggsave('Fig6_ensemble_allmodels_boxplot_GCMRCM2_v2.pdf', p_all, width =500,
       height = 550, units = 'mm')
