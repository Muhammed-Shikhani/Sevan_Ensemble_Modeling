...
dir.data <- "/home/shikhani/Documents/LER_Sevan_cordex_swr/rdata_BiasCorrected_hist_used/"
rdata_files <- list.files( "/home/shikhani/Documents/LER_Sevan_cordex_swr/rdata_BiasCorrected_hist_used/")
length(rdata_files)
sim_folder <- "/home/shikhani/Documents/LER_Sevan_cordex_swr/"
...
for(m in 1:length(model)){
  for(j in 1:length(my_rcps)){
    ...
    for (ii in 1:length(files_rcp)) {
      ...
      input_yaml(file = "LakeEnsemblR.yaml", label = 'output', key = 'file', value = paste("ensemble_output", model[m],cordex_name, sep = "_"))
      config_file <- 'LakeEnsemblR.yaml'
      export_config(config_file = config_file, model = model[m])
      
      
      run_ensemble(config_file = config_file, model = model[m],parallel = T,verbose = F)
      
      t=t+1
      end.time <- Sys.time()
      time.taken <- end.time - start.time
      print(paste(t, cordex_name, sep = " "))
      print(time.taken)
    }
    print(my_rcps[j])
  }
