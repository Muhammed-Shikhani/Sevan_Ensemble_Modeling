...
p55 <- strat_ag %>% filter(model == "Ensemble_Mean") %>%
  ...
  scale_fill_manual("Period", values = mycol2, guide = "none")+ 
  facet_wrap(. ~ variable, ncol = 1, scales = "free_y", strip.position = "left", 
             labeller = as_labeller(c(MaxStratDur = "Max Stratification Duration (day)", StratStart = "Stratification Onset (doy)", StratEnd = "Stratification End (doy)", Schmidt_Stability = "Schmidt Stability J/m2"))) +
  labs(y = NULL, x = "Date")
