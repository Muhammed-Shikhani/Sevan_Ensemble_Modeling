...
calc_trend <- function(x) {
  ...
  tau <- as.vector(Kendall::MannKendall(x$anomaly)$tau)
  
  # Calculate Sen's slope
  slope <- as.vector(trend::sens.slope( x$anomaly)$est[1])*10
...
period_list <-unique(dat_anomaly_withmean$period)
variable_list <- unique(dat_anomaly_withmean$variable)
...
for (period in period_list) {
  dat_p <- dat_anomaly_withmean[which(dat_anomaly_withmean$period== period),]
...
trends_mean[which(trends_mean$model%in% c("Ensemble_Mean")), ]
subset_trends <- trends_mean[which(trends_mean$model%in% c( "Climate_Model","Ensemble_Mean")), ]
...
subset_table <- averages_ordered %>%
  filter(model %in% c("Climate_Model", "Ensemble_Mean"))
...
merged_table <- merge(subset_table, subset_trends, by = c("model", "variable", "period"), all = TRUE)
...
merged_ordered <- merged_ordered[, -which(names(merged_ordered) == "model")]
merged_ordered$anomaly_period <- ifelse(merged_ordered$period == "historical", "1995-2005", "2089-2099")


merged_ordered <- merged_ordered %>%
  select(period, variable, slope, anomaly_period, everything())


merged_ordered_rounded <- merged_ordered

for(i in c(3,5:11)){
  merged_ordered_rounded[,i] <- round(merged_ordered[,i],2)
  
}
merged_ordered_rounded 
# Convert merged_ordered to a table
table_merged <- xtable(merged_ordered_rounded)
